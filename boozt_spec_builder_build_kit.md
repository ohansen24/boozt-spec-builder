...
2. Generic resolver (any brand, and validators): web search for "{gtin13}" and "{ean12} {brand}", collect candidate URLs, fetch, parse JSON-LD/microdata, accept only documents whose gtin/gtin12/gtin13/ean equals the item. Extract Product.name, color/variant, size, and ingredient fields when present. POLICY (2026-07-03): where no brand source is EAN-addressable (e.g. Marc Jacobs fragrances), the generic resolver's retailer data is the primary evidence, and GREEN requires agreement of TWO independent retailer families — never one.
