...
| **maria_nila** | Shopify (handle.js barcode index) + retailer-primary closing pass | ✅ 78/109 brand + 31/31 retailer | ✅ brand; retailer size gap (in names, not extracted) | ✅ brand PDPs; retailer non-shade | ✅ 62 brand + 20 retailer families (11 still none) | OR26BZNX0001 emitted; 2 rows got non-EN retailer names — dedupe/lang-prefer follow-up |
| **benefit** | SFRA **catalog-index**: upc→variant_code via master-PDP datalayer, Product-Variation per variant for shade/size (barcode is NOT the pid). **Swatch hex: ✅ native** (dwvar color value) — drives Stage-2 color-code proposer | ✅ 63/63 resolved (single family → yellow) | 🟡 28/63 via LF; no structured on-site volume | ✅ 63/63 site displayValue, numbered-shade shade_format; **color_code 48/63 proposed** (16 two-signal + 14 hex-only + rules), 13 word/hex-disagree + 2 no-signal → Felina | 🟡 22/63 via lookfantastic JSON (EU); rest retailer-coverage gap | **Gates A/B/C PASSED (OR26BZFT0001)**; DE brand-family for discontinued shades; color-code auto-proposers (Stage 1 word + Stage 2 hex) — all yellow proposals, Felina confirms |
