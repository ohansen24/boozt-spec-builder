...
| **benefit** | SFRA **catalog-index**: upc→variant_code via master-PDP datalayer, Product-Variation per variant for shade/size (barcode is NOT the pid) | ✅ 63/63 resolved (single family → yellow) | ⛔ no structured per-variant volume on-site (label "Full Size"/"Mini" only) → retailer/Felina | ✅ 63/63 site displayValue; color_code fail-closed→Felina by design, concealer→Foundation/1018 | ⚠️ sparse on-site → retailer/validator pass needed | **Gates A/B/C PASSED (OR26BZFT0001)**: 0 anchor rejections, 63/63 category, independent 63/63 cross-check; INCI+size via retailer pass; color_name QA list delivered |
